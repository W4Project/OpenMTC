...
class TestGUI(XAE):
    ...
    def handle_measurements(self, container, data):
        ...
        # simple logic to control the motor
        if type_ == 'nutrient solution temp':
            if value > 24:
                data = {'Power': 'ON'}
                print('Nutrient Sol Temp = %s > 24. Too hot! \n Turning Water Tank Motor ON' % value)
            elif value < 15:
                data = {'Power': 'ON'}
                print('Nutrient Sol Temp = %s < 15. Too cold! \n Turning Water Tank Motor ON' % value)
            else:
                data = {'Power': 'OFF'}
                print('15 < Nutrient Sol Temp = %s < 24. Just right! \n Turning AirConditioning OFF' % value)
            
            # push the new command based on temperature measurements to all known AirCon actuators
            for actuator in self.actuators:
                if 'Motor' in actuator:
